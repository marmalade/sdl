...
int timerCallback(void* unused, SDL_TimerID id)
{
	Uint32 next = id->cb(id->interval, id->param);

	if( next )
	{
		id->interval = next;
		s3eTimerSetTimer(id->interval, (s3eCallback)timerCallback, id);
	}
...
static SDL_TimerID SDL_AddTimerInternal(Uint32 interval, SDL_NewTimerCallback callback, void *param)
{
	...
	s3eTimerSetTimer(t->interval, (s3eCallback)timerCallback, t);
#ifdef DEBUG_TIMERS
...
/*	SDL_TimerID t;
	...
	SDL_mutexP(SDL_timer_mutex);*/
	SDL_TimerID t = SDL_AddTimerInternal(interval, callback, param);
//	SDL_mutexV(SDL_timer_mutex);
	return t;
}
...
/*	for (t = SDL_timers; t; prev=t, t = t->next ) {
	...
	return removed;
	*/
	s3eTimerCancelTimer((s3eCallback)timerCallback, id);
	return 1;
}
